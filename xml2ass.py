...
def xml2ass(name):
    ...
    Original Script: 根据 {name} 的弹幕信息，由 https://github.com/tiansh/us-danmaku 生成
    ScriptType: v4.00+
    Collisions: Normal
    PlayResX: 560
    PlayResY: 420
    ...
    Style: Fix,黑体,25,&H66FFFFFF,&H66FFFFFF,&H66000000,&H66000000,1,0,0,0,100,100,0,0,1,2,0,2,20,20,2,0
    Style: R2L,黑体,25,&H66FFFFFF,&H66FFFFFF,&H66000000,&H66000000,1,0,0,0,100,100,0,0,1,2,0,2,20,20,2,0

    [Events]
    Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text
    """.format(name=name)

    dom = xml.dom.minidom.parse('【正道的光】马保国被人民日报点名批评.cmt.xml')

    root = dom.documentElement
    dan_list = root.getElementsByTagName('d')

Dialogue: 0,{time},{time+8},{type},,20,20,2,,{\move(572.5,50,-12.5,50)}{text}


<d p="1606820944,0,85bce90c,41774622460870663">悲</d>
Dialogue: {\move(572.5,50,-12.5,50)}{text}

<d p="1606822383,0,99f7ac47,41775377190223879"></d>
Dialogue: {\move(635,75,-75,75)\c&H0203FE}

"16777215,1606634528,0,aa754386,41676887057498115"
{\move(610,50,-50,50)}

    # 排序
    for dan in dan_list:
        param=dan.getAttribute('p').split(',')
        time=param[0]# time
        type='R2L' if param[3]==16777215 else 'Fix'
        time=param[4]
        time=param[5]
        time=param[6]
        time=param[7]
        text=param.firstChild.data
